...
public interface AccountsApi {
    ...
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Account succesfully created.", response = Void.class),
        @ApiResponse(code = 400, message = "Error creating the account", response = Void.class) })
    @RequestMapping(value = "/accounts",
        method = RequestMethod.POST)
    ResponseEntity<Void> createAccount(@ApiParam(value = "The account to be created."  ) @RequestBody AccountDetails accountDetails);
    ...
    ResponseEntity<AccountDetails> getAccountDetails(@ApiParam(value = "The person's accountNumber",required=true ) @PathVariable("accountNumber") Integer accountNumber);


    @ApiOperation(value = "Gets account details", notes = "Returns a list containing all account details", response = Accounts.class, tags={  })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "A list of Accounts", response = Accounts.class) })
    @RequestMapping(value = "/accounts",
        method = RequestMethod.GET)
